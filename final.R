...
sum(is.na(merge.data_gb$vertical_max))/nrow(merge.data_gb)
sum(is.na(merge.data_gb$bench))/nrow(merge.data_gb)
sum(is.na(merge.data_gb$agility))/nrow(merge.data_gb)
sum(is.na(merge.data_gb$sprint))/nrow(merge.data_gb)
sum(is.na(merge.data_gb$hand_length))/nrow(merge.data_gb)
sum(is.na(merge.data_gb$hand_width))/nrow(merge.data_gb)
#
...
set.seed(100)
#merge.data_gb <- ungroup(merge.data_gb)
#new_data_cat <- select(merge.data_gb, Pos, body_fat, hand_width, hand_length, height_with_shoes, wingspan, free_average)

# new_data_cat to a classification problem
...
train_set<-new_data_cat[train_idx,-1]
test_set<- new_data_cat[-train_idx,-1] 
...
rf_free_cat.b <- train(free_cat~free_total+wingspan+wingspan_ratio+BMI+agility,
                     data=train_set_withoutna, method="rf",
                     trControl=tc1)
...
for(iy in c(1:nrow(paramTable))){
  ...
  cvOutput <- cvOutput %>%
    bind_rows(tibble(paramsNum = iy,
                     ...
                     bestCvlogloss = cvResult$evaluation_log$train_mlogloss_mean[bestIteration]))
...
View(freetotal_relation)
View(test_set_withoutna)
new_testing_set=separate_rows(test_set_withoutna,Pos,convert=T)
freetotal_relation<-new_testing_set%>%select(free_cat,free_total)%>%group_by(free_cat)%>%summarise(free_toal1=mean(free_total))
View(freetotal_relation)
